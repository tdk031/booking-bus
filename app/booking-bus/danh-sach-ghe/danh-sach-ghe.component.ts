...
export class DanhSachGheComponent implements OnInit {
  @ViewChildren(GheComponent) huyGhe: QueryList<GheComponent> = new QueryList();
  ...
  huyDatGhe(gheDaDat: any) {
    for (let index in this.danhSachGheDaDat) {
      if (this.danhSachGheDaDat[index].SoGhe === gheDaDat.SoGhe) {
        this.danhSachGheDaDat.splice(parseInt(index), 1);
      }
    }

    this.huyGhe?.forEach((ghe) => {
      if (ghe.chair.SoGhe === gheDaDat.SoGhe) {
        ghe.chair.TrangThai = false;
      }
    });
  }

  tongTien() {
    return this.danhSachGheDaDat.reduce(
      (tienTong: number, ghe: any, index: number) => (tienTong += ghe.Gia),
      0
    );
  }
}
